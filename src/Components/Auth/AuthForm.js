...
function AuthForm(props) {
  ...
  const afterAuth = (user, mode = "Signup") => {
    ...
    props.setAdminAccess(user["isAdmin"])
    let redirect = '/'
    setTimeout(() => {
      console.log("redirecting after",mode)
  ...
  const catchError =(err)=>{
    if (err.response) {
      changePopUp(prev => ({ ...prev, message: `${err.response.data}`, severity: "error" }))
      changeAuthErr(err.response.data);
    }
    else {
  ...
  return (
    <Container className="auth-form-container" component="main" maxWidth="xs" >
      ...
      <div className={classes.paper}>
        <Typography component="h1" variant="h5" style={{ paddingTop: "20px", paddingLeft: "8px" }}>
